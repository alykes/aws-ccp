...
- `CloudHSM`  
  - KMS manages AWS manages the software encryption  
...
- `CMK (Customer Master Keys)`  
  ...
  - **AWS managed CMK**:  
    - Created, maanged and used on the customer's behalf by AWS  
    - Used by AWS Services (s3, ebs, redshift)    
  - **AWS owned CMK**:  
    - Collection of CMKs taht an AWS Service owns and manages to use in multiple accounts  
...
- `Secrets Manager`  
  ...
  - auto-generate secrets on roation (uses lambda)  
  - integration with Amazon RDS (MySQL, PostgreSQL, Aurora)  
  - secrets are encrypted using KMS  
  - Mostly meant for RDS integration but you can store any other secret in there as well  
  - `Exam Tip`: If you see a question about rotating keys for RDS and where it will be stored, think secrets manager  
...
- `GuardDuty`  
  ...
  - `Exam Tip`: **Can protect against CryptoCurrency attakcs (has a dedicated method for detection)**  
...
- a risk score is associated with all vulnerabilities for proritisation  
...
- Abusive or prohibited behaviours include:  
  ...
  - Port Scanning - sending packetsto your ports  
...
- Root account has complete unhindered access toa ll AWS services and resources  
...
- Actions that only the root user can perform are:  
  - `Exam Tip` Take not of the items listed in bold  
...
- Define **Zone of Trust** on the AWS Account of AWS Organisation  
